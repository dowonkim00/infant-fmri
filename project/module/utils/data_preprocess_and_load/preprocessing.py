...
def main():
    ...
    dataset_name = 'dHCP'
    load_root = '/storage/bigdata/dHCP/fmriprep/1.rs_fmri/4.cleaned_image' # This folder should have fMRI files in nifti format with subject names. Ex) sub-01.nii.gz
    scaling_method = 'z-norm' # choose either 'z-norm'(default) or 'minmax'.
    save_root = f'/storage/bigdata/dHCP/fmriprep/1.rs_fmri/7.{dataset_name}_MNI_to_TRs_{scaling_method}'
    ...
    for filename in sorted(filenames):
        subj_name = re.match(r'^(sub-\w+_ses-\d+)_', filename).group(1)
        ...
        expected_seq_length = 2300 # Specify the expected sequence length of fMRI for the case your preprocessing stopped unexpectedly and you try to resume the preprocessing.
